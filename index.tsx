...
// Removed: import { getAnalytics, Analytics } from "firebase/analytics";
import {
    getAuth,
    onAuthStateChanged,
...
// Removed: const analytics: Analytics = getAnalytics(app);
...
let currentView: AppView = AppView.LOGIN;
let currentUser: User | null = null; // Holds the current Firebase user
...
document.addEventListener("DOMContentLoaded", () => {
    // View elements
    const loginView = document.getElementById(AppView.LOGIN) as HTMLElement;
    const dashboardView = document.getElementById(AppView.DASHBOARD) as HTMLElement;
    const toolView = document.getElementById(AppView.TOOL) as HTMLElement;
    const contactView = document.getElementById(AppView.CONTACT) as HTMLElement;
    const settingsView = document.getElementById(AppView.SETTINGS) as HTMLElement;
    ...
    const form = document.getElementById("energyDataForm") as HTMLFormElement;
    const fieldsets = Array.from(document.querySelectorAll<HTMLElement>('#tool-view .form-step'));
    const progressBar = document.getElementById('progress') as HTMLElement;
    const progressSteps = Array.from(document.querySelectorAll<HTMLElement>('#tool-view .progress-step'));
    const resultSection = document.getElementById('resultSection') as HTMLElement;
    const resultDiv = document.getElementById('result') as HTMLElement;
    const numberOfPanelsSelect = document.getElementById('numberOfPanels') as HTMLSelectElement;
    const clearCurrentFormButton = document.getElementById('clear-current-form-btn') as HTMLButtonElement;

    // Navigation buttons
    const loginBtn = document.getElementById('login-btn') as HTMLButtonElement;
    const logoutBtn = document.getElementById('logout-btn') as HTMLButtonElement;
    const addClientNavBtn = document.getElementById('add-client-nav-btn') as HTMLButtonElement;
    const contactNavBtn = document.getElementById('contact-nav-btn') as HTMLButtonElement;
    const settingsNavBtn = document.getElementById('settings-nav-btn') as HTMLButtonElement;
    const backToDashboardFromToolBtn = document.getElementById('back-to-dashboard-from-tool-btn') as HTMLButtonElement;
    const backToDashboardFromContactBtn = document.getElementById('back-to-dashboard-from-contact-btn') as HTMLButtonElement;
    const backToDashboardFromSettingsBtn = document.getElementById('back-to-dashboard-from-settings-btn') as HTMLButtonElement;

    const saveClientDataBtn = document.getElementById('save-client-data-btn') as HTMLButtonElement;

    // Contact Form
    const contactForm = document.getElementById('contact-form') as HTMLFormElement;
    const contactFormFeedback = document.getElementById('contact-form-feedback') as HTMLParagraphElement;
    ...
    const clearAllAppDataBtn = document.getElementById('clear-all-app-data-btn') as HTMLButtonElement;
    ...
    function switchView(targetView: AppView) {
    ...
    if (loginBtn) {
        loginBtn.addEventListener('click', async () => {
            const emailInput = document.getElementById('username') as HTMLInputElement; // ID is username, but it's for email
    ...
    function getElementValueAsNumber(id: string, defaultValue = 0): number {
    ...
    function initializeForm() {
        fieldsets.forEach((fs, idx) => {
            ...
            fs.querySelectorAll<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>('input, select, textarea').forEach(input => {
        ...
        const calcBtn = fieldsets[currentStep]?.querySelector<HTMLButtonElement>('#calculate-btn.next-btn');
    ...
    function setErrorMessage(field: HTMLElement, message: string) {
        ...
        if (message) {
            ...
            (field as HTMLInputElement).setAttribute('aria-invalid', 'true');
        } else {
            (field as HTMLInputElement).removeAttribute('aria-invalid');
        }
    ...
    function validateCurrentStep(): boolean {
        ...
        const requiredFields = currentFieldset.querySelectorAll<HTMLInputElement|HTMLSelectElement>('input[required], select[required]');
        let isValid = true;
        ...
        const loanTermField = document.getElementById('loanTerm') as HTMLInputElement;
        ...
        const installationAddressCheckbox = document.getElementById('installationAddressDifferent') as HTMLInputElement;
        if (installationAddressCheckbox && installationAddressCheckbox.checked) {
            const section = document.getElementById('installationAddressSection') as HTMLElement;
            section.querySelectorAll<HTMLInputElement>('input[required]').forEach(field => {
                 if (!field.value.trim()) {
                    isValid = false;
    ...
    function setButtonLoadingState(button: HTMLButtonElement, loadingText: string) {
        button.disabled = true;
        const textSpan = button.querySelector('.btn-text') as HTMLSpanElement;
        const mainIcon = button.querySelector('.icon-main') as HTMLElement;
        const loadingIcon = button.querySelector('.icon-loading') as HTMLElement;
    ...
    function resetButtonState(button: HTMLButtonElement, defaultText: string, defaultIconClass: string) {
            ...
            if (e.key === 'Enter') {
                if (!((e.target as HTMLElement).tagName === 'TEXTAREA' || (e.target as HTMLElement).tagName === 'BUTTON')) {
                    ...
                    if (calculateBtnInStep) {
                        ...
                        const nextButton = activeFieldset.querySelector<HTMLButtonElement>('.next-btn');
                        if (nextButton) {
                            nextButton.click();
                        }
                    }
    ...
    const aantalBewonersSelect = document.getElementById('aantalBewoners') as HTMLSelectElement;
    const gasConsumptionInput = document.getElementById('gasConsumption') as HTMLInputElement;
    const electricityConsumptionInput = document.getElementById('electricityConsumption') as HTMLInputElement;
    ...
    if (aantalBewonersSelect) {
        aantalBewonersSelect.addEventListener('change', () => {
            ...
            if (gasConsumptionInput && bewoners in placeholders.gas) {
                ...
                if (!gasConsumptionInput.value && !gasConsumptionInput.readOnly) gasConsumptionInput.value = (placeholders.gas as any)[bewoners].toString();

            }
            if (electricityConsumptionInput && bewoners in placeholders.elektra) {
                electricityConsumptionInput.placeholder = (placeholders.elektra as any)[bewoners].toString();
                 if (!electricityConsumptionInput.value && !electricityConsumptionInput.readOnly) electricityConsumptionInput.value = (placeholders.elektra as any)[bewoners].toString();
            }
        });
    }

    const bouwjaarInput = document.getElementById("bouwjaar") as HTMLInputElement;
    const woningtypeSelect = document.getElementById("woningtype") as HTMLSelectElement;

    function updateInsulationOptionsVisibility() {
        const bouwjaar = getElementValueAsNumber("bouwjaar");
        const woningtype = getElementValue("woningtype") as keyof typeof insulationData;
        const insulationNoteEl = document.getElementById("insulation-advice-note") as HTMLElement;
        ...
        insulationFieldIds.forEach((fieldId, index) => {
            const selectElement = document.getElementById(fieldId) as HTMLSelectElement | null;
            if (!selectElement) return;

            const originalSelectedValue = selectElement.value;
            selectElement.innerHTML = '';

            const insuKey = insulationTypeKeys[index] as keyof typeof insulationData[keyof typeof insulationData];
            const data = insulationData[woningtype]?.[insuKey] as any;
    ...
    function calculateHeatPumpAdvice(livingArea: number, bouwjaar: number, gasConsumption: number): string {
    ...
    function calculateInsulationCosts(livingArea: number, woningtypeKey: string, selections: Record<string, string>): Record<string, { brutoPrice: number, subsidie: number, gasSavingApplied: number, label: string }> {
        const results: Record<string, { brutoPrice: number, subsidie: number, gasSavingApplied: number, label: string }> = {};
        const typeData = insulationData[woningtypeKey as keyof typeof insulationData];
        if (!typeData || !livingArea) return results;

        for (const [key, selectionValue] of Object.entries(selections)) {
            if (selectionValue === "Yes") {
                const insuKey = key as keyof typeof typeData;
                const insuDetails = typeData[insuKey] as any;
                if (insuDetails) {
                 ...
                 results[key] = { brutoPrice: 0, subsidie: 0, gasSavingApplied: 0, label: typeData[key as keyof typeof typeData]?.label || key };
            }
        }
        return results;
    }
    ...
    function collectAndDisplayData() {
        ...
        const formData = {
            ...
            heatPumpType: getElementValue("heatPumpType") as keyof typeof heatPumpData,
            heatPumpSize: getElementValue("heatPumpSize"),
            numberOfPanels: getElementValueAsNumber("numberOfPanels"),
            meterkastNieuw: getElementValue("meterkastNieuw") as keyof typeof MeterkastKosten,
            aircoType: getElementValue("aircoType") as keyof typeof AircoPrijzen,
    ...
    function displayResults(results: any) {
        if(!resultDiv) return;
        const f = (num: number) => num.toFixed(2).replace('.', ',');
        const kwh = (num: number) => Math.round(num).toLocaleString('nl-NL');
        const m3 = (num: number) => Math.round(num).toLocaleString('nl-NL');
    ...
    document.body.addEventListener('click', function(event) {
        ...
        if (target && target.id === 'toggle-results') {
            const important1Items = document.querySelectorAll<HTMLElement>('.important-1');
            const important2Items = document.querySelectorAll<HTMLElement>('.important-2');
            const toggleButton = target as HTMLButtonElement;
    ...
    const installationAddressCheckbox = document.getElementById('installationAddressDifferent') as HTMLInputElement;
    const installationAddressSection = document.getElementById('installationAddressSection') as HTMLElement;
    if (installationAddressCheckbox && installationAddressSection) {
        installationAddressCheckbox.addEventListener('change', () => {
            ...
            installationAddressSection.querySelectorAll<HTMLInputElement>('input').forEach(input => {
                if (isChecked) {
    ...
    async function saveClient(clientData: Omit<Client, 'id'>): Promise<string | null> {
    ...
    async function renderClientList() {
        ...
        clientListContainer.querySelectorAll<HTMLButtonElement>('.view-quote-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const clientId = btn.dataset.clientId;
                if (clientId) showClientQuote(clientId);
            });
        });
        clientListContainer.querySelectorAll<HTMLButtonElement>('.send-webhook-btn').forEach(btn => {
    ...
    async function showClientQuote(clientId: string) {
    ...
    async function sendClientDataToWebhookAndArchive(clientId: string) {
        ...
        const clientData = clientDocSnap.data() as Omit<Client, 'id'>;
        ...
        saveClientDataBtn.addEventListener('click', async () => {
            ...
            if (validateCurrentStep()) {
                ...
                const quoteData = {
                    ...
                    installationAddressDifferent: (document.getElementById('installationAddressDifferent') as HTMLInputElement).checked,
    ...
    function fQuote(num: number | undefined, decimals = 2): string {
        if (typeof num === 'undefined' || isNaN(num)) return 'N/A';
        return num.toFixed(decimals).replace('.', ',');
    }
    function kwhQuote(num: number | undefined): string {
        if (typeof num === 'undefined' || isNaN(num)) return 'N/A';
        return Math.round(num).toLocaleString('nl-NL');
    }

    function generateQuoteHTML(quoteData: any, resultsData: any): string {
        ...
        if (resultsData?.insulationChosenDetails) {
            Object.entries(resultsData.insulationChosenDetails).forEach(([key, detail]: [string, any]) => {
        ...
        if (resultsData?.heatPumpChosen && resultsData.heatPumpChosen !== "Geen") {
            const hpKey = quoteData.heatPumpType as keyof typeof heatPumpData;
        ...
        if (quoteData.meterkastNieuw !== "geen_vervanging") {
            const mkDetails = MeterkastKosten[quoteData.meterkastNieuw as keyof typeof MeterkastKosten];
        ...
        if (quoteData.aircoType !== "none") {
             const acDetails = AircoPrijzen[quoteData.aircoType as keyof typeof AircoPrijzen];
    ...
    async function saveFormToFirestore() {
        ...
        const formDataToSave: Record<string, string | boolean> = {};
        form.querySelectorAll<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>('input, select, textarea').forEach(element => {
            if (element.id) {
                if (element.type === 'checkbox') {
                    formDataToSave[element.id] = (element as HTMLInputElement).checked;
                } else {
    ...
    async function loadFormFromFirestore() {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (formData) {
                    let aantalBewonersLoadedValue: string | null = null;
                    let bouwjaarLoadedValue: string | null = null;
                    let woningtypeLoadedValue: string | null = null;
                    let heatPumpTypeLoadedValue: string | null = null;
                    let installationAddressDifferentLoadedValue: boolean | null = null;

                    Object.keys(formData).forEach(id => {
                        const element = document.getElementById(id) as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement;
                        if (element) {
                            if (element.type === 'checkbox') {
                                (element as HTMLInputElement).checked = formData[id] as boolean;
                                if (id === 'installationAddressDifferent') {
                                    installationAddressDifferentLoadedValue = formData[id] as boolean;
                                }
                            } else {
                                element.value = formData[id] as string;
                            }
                            if (id === 'aantalBewoners') aantalBewonersLoadedValue = formData[id] as string;
                            if (id === 'bouwjaar') bouwjaarLoadedValue = formData[id] as string;
                            if (id === 'woningtype') woningtypeLoadedValue = formData[id] as string;
                            if (id === 'heatPumpType') heatPumpTypeLoadedValue = formData[id] as string;
                    ...
                    if (heatPumpTypeLoadedValue) (document.getElementById('heatPumpType') as HTMLSelectElement)?.dispatchEvent(new Event('change', { bubbles: true }));
                    if (installationAddressDifferentLoadedValue !== null) (document.getElementById('installationAddressDifferent') as HTMLInputElement)?.dispatchEvent(new Event('change', { bubbles: true }));
                }
            }
        } catch (error) {
    ...
    if (clearCurrentFormButton) {
        clearCurrentFormButton.addEventListener('click', async () => {
            ...
            if (confirm("Weet u zeker dat u de huidige formulierinvoer wilt wissen? Opgeslagen klantgegevens blijven behouden.")) {
                ...
                (document.getElementById('heatPumpType') as HTMLSelectElement)?.dispatchEvent(event);
                (document.getElementById('installationAddressDifferent') as HTMLInputElement)?.dispatchEvent(event);
                currentStep = 0;
                lastCalculationResults = null;
                if(resultSection) resultSection.classList.add('hidden');
                if(resultDiv) resultDiv.innerHTML = '<p>Uw resultaten worden berekend...</p>';
    ...
    async function deleteCollection(collectionRef: CollectionReference, batchSize = 10) {
    ...
    async function deleteQueryBatchModular(query: Query, resolve: () => void, reject: (reason?: any) => void) {
        try {
